...
Emit a MINIMUM of 8 elements with bboxes for any non-trivial scene (i.e. more than a single isolated object on a blank field). Target 15+ for populated scenes (interiors, streets, markets, landscapes with foreground/midground/background layers). If your element list is below 8, re-examine the scene and add dummy bbox elements with details
